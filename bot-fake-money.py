...
try:
    ...
    printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} If that was with real money, orders would be sent here for {round(total_crypto/len(echanges),3)} {currentPair[:-5]} at {average_first_buy_price}.")
        ...
        try:
            ...
            if max_bid_ex != min_ask_ex and profit_with_fees_usd > float(criteria_usd) and profit_with_fees_pct > float(criteria_pct) and prec_ask_price != min_ask_price and prec_bid_price != max_bid_price and usd[min_ask_ex] >= crypto_per_transaction * min_ask_price * (1+fees[min_ask_ex]['give']) and crypto[max_bid_ex] >= crypto_per_transaction * (1+fees[max_bid_ex]['give']):
                ...
                if profit_with_fees_usd < 0:
                    ...
                    color = Fore.WHITE
                print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Best opportunity: {color}{round(profit_with_fees_usd,4)} USD {Style.RESET_ALL}(with fees)       buy: {min_ask_ex} at {min_ask_price}     sell: {max_bid_ex} at {max_bid_price}")
